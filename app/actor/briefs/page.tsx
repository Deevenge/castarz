...
import { BriefcaseBusiness, CalendarDays, CheckCircle2, Clock3, MapPin, WalletCards, XCircle } from "lucide-react";
...
type Booking = { briefTitle: string; location: string; shootDate: string; rate: string; agencyName: string };

export default function MyApplicationsPage() {
  const { user } = useAuth(); const [applications, setApplications] = useState<Application[]>([]); const [briefs, setBriefs] = useState<Record<string, AgentBrief>>({}); const [bookings, setBookings] = useState<Record<string, Booking>>({});
  useEffect(() => {
    if (!user) return;
    ...
    return () => { stopApps(); stopBookings(); };
  }, [user]);
  return <div className="mx-auto max-w-3xl"><header><p className="text-sm font-bold tracking-[0.18em] text-brand-blue">MY APPLICATIONS</p><h1 className="mt-1 text-3xl font-bold">Your casting journey.</h1><p className="mt-2 text-slate-600">Follow every application from submission to the final agency decision.</p></header><section className="mt-7 space-y-4">{applications.map((application) => { const brief = briefs[application.briefId]; const booking = bookings[application.id]; return <article key={application.id} className="rounded-3xl bg-white p-5 shadow-sm ring-1 ring-brand-silver/70"><div className="flex flex-wrap items-start justify-between gap-4"><div><p className="text-sm font-bold text-brand-blue">{brief?.agencyName || booking?.agencyName || "Casting agency"}</p><h2 className="mt-1 text-xl font-bold text-brand-navy">{brief?.title || "Loading brief…"}</h2><p className="mt-2 text-sm text-slate-600">{statusCopy(application.status)}</p>{application.status === "booked" && booking && <div className="mt-4 grid gap-2 rounded-2xl bg-emerald-50 p-4 text-sm font-semibold text-emerald-800"><p className="flex items-center gap-2"><MapPin className="size-4" />{booking.location || "Location to follow"}</p><p className="flex items-center gap-2"><CalendarDays className="size-4" />{booking.shootDate || "Date to follow"}</p><p className="flex items-center gap-2"><WalletCards className="size-4" />{booking.rate || "Rate to follow"}</p></div>}</div><Status status={application.status} /></div></article>; })}{!applications.length && <div className="rounded-3xl border-2 border-dashed border-brand-silver bg-white p-10 text-center"><BriefcaseBusiness className="mx-auto size-9 text-brand-blue" /><h2 className="mt-4 text-xl font-bold">No applications yet</h2><p className="mt-2 text-slate-600">Explore open opportunities on Home and apply when a role fits.</p></div>}</section></div>;
}
