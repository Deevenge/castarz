...
export default function ActorDashboardPage() {
  ...
  useEffect(() => onSnapshot(query(collection(db, "briefs"), where("status", "==", "published")), (snapshot) => setBriefs(snapshot.docs.map((item) => briefFromDocument(item.id, item.data())))), []);
  useEffect(() => {
    if (!user) return;
  ...
  useEffect(() => {
    if (!user) return;
    return onSnapshot(query(collection(db, "connections"), where("actorUid", "==", user.uid)), (snapshot) => setConnectedAgencyIds(snapshot.docs.filter((item) => item.data().status === "approved").map((item) => item.data().agencyId).filter((id): id is string => typeof id === "string")));
  }, [user]);
