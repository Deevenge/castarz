    ...
    <div className="relative flex size-28 shrink-0 items-center justify-center overflow-hidden rounded-full bg-brand-ice ring-4 ring-white sm:size-32">
      {src ? (
        <button type="button" onClick={() => setViewerOpen(true)} className="absolute inset-0 cursor-zoom-in" aria-label={`View ${name} profile photo`}>
          <Image src={src} alt={name} fill unoptimized className="object-cover object-top" />
        </button>
      ) : <UserRound className="m-auto size-12 text-brand-blue" />}
      {onUpload && (
        <label className="absolute bottom-1 right-1 z-10 flex size-9 cursor-pointer items-center justify-center rounded-full bg-brand-navy text-white shadow-lg">
          <Camera className="size-4" />
          <input type="file" accept="image/*" onChange={onUpload} className="sr-only" />
        </label>
      )}
    </div>
    {src && viewerOpen && <PhotoLightbox photo={src} label={`${name} profile photo`} close={() => setViewerOpen(false)} />}
    </>
  );
}
...
export function PhotoGrid({ photos, emptyLabel }: { photos: string[]; emptyLabel: string }) {
  ...
  return (
    <>
      <div className="grid grid-cols-3 gap-1 sm:gap-1.5">
        {photos.map((photo, index) => (
          <button type="button" key={`${photo.slice(-20)}-${index}`} onClick={() => setViewer(index)} className="group relative aspect-square overflow-hidden bg-brand-ice text-left first:rounded-tl-2xl last:rounded-br-2xl">
            <Image src={photo} alt="" fill unoptimized className="object-contain object-top transition duration-300 group-hover:scale-105" />
      ...
      {activePhoto && <PhotoLightbox photo={activePhoto} label={`${(viewer ?? 0) + 1} / ${photos.length}`} close={() => setViewer(null)} previous={() => setViewer((current) => current === null ? null : (current - 1 + photos.length) % photos.length)} next={() => setViewer((current) => current === null ? null : (current + 1) % photos.length)} />}
    </>
  );
}

export function PhotoLightbox({ photo, label, close, previous, next }: { photo: string; label: string; close: () => void; previous?: () => void; next?: () => void }) {
  return (
    <div className="fixed inset-0 z-[70] flex items-center justify-center bg-black/90 p-4">
      <div className="absolute left-5 top-5 rounded-full bg-white/15 px-3 py-1.5 text-sm font-bold text-white">{label}</div>
      <button type="button" onClick={close} className="absolute right-5 top-5 flex size-11 items-center justify-center rounded-full bg-white/15 text-white hover:bg-white/25" aria-label="Close photo"><X className="size-6" /></button>
      <a href={photo} download="castarz-photo.jpg" className="absolute right-20 top-5 flex size-11 items-center justify-center rounded-full bg-white/15 text-white hover:bg-white/25" aria-label="Download photo"><Download className="size-5" /></a>
      {previous && <button type="button" onClick={previous} className="absolute left-3 flex size-12 items-center justify-center rounded-full bg-white/15 text-white hover:bg-white/25 sm:left-8" aria-label="Previous photo"><ChevronLeft className="size-7" /></button>}
      <div className="max-h-[82dvh] max-w-[88vw] overflow-auto rounded-2xl">
        <Image src={photo} alt={label} width={1200} height={1200} unoptimized className="h-auto max-h-[82dvh] w-auto max-w-full cursor-zoom-in rounded-2xl object-contain transition-transform hover:scale-125" />
      </div>
      {next && <button type="button" onClick={next} className="absolute right-3 flex size-12 items-center justify-center rounded-full bg-white/15 text-white hover:bg-white/25 sm:right-8" aria-label="Next photo"><ChevronRight className="size-7" /></button>}
    </div>
  );
}
